...
export default function TimetableForm({ initialData }: TimetableFormProps) {
  ...
  const handleAsrSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to update Asr timing';
      showToast(msg, 'error');
    } finally {
      setTimingsLoading(false);
    }
  ...
  const handleGymSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to update gym preference';
      showToast(msg, 'error');
    } finally {
      setTimingsLoading(false);
    }
  ...
  const handleActivitiesSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to update activities';
      showToast(msg, 'error');
    } finally {
            ...
            <select
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  const msg = err instanceof Error ? err.message : 'Failed to update method';
                  showToast(msg, 'error');
                }
              }}
              className="search-input"
          ...
          {[
            ...
            <div
              ...
              onClick={() => setEditingTiming({ 
                ...
                label: item.label, 
                icon: item.icon, 
                value: ('rawVal' in item) ? item.rawVal as string | number : item.val 
              })}
              style={{
                display: 'flex',
                alignItems: 'center',
